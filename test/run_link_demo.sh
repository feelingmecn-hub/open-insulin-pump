...
# ---- 1) 确保联调版二进制存在且为最新 ----
# 判定以"构建模式标记"为准: 只有 .built_link_mode 存在且源码未变才跳过,
# 否则一律重建联调版 (避免误用 mock 版二进制导致演示失效)。
LINK_MARK="$BUILD/.built_link_mode"
need_build=0
if [ ! -f "$LINK_MARK" ]; then
  need_build=1
  echo "[1/4] 当前二进制非联调版 (无标记/为 mock), 需要构建联调版"
elif [ "${FORCE:-0}" = "1" ]; then
  need_build=1
  echo "[1/4] FORCE=1, 强制重新构建联调版"
else
  # 比较联调源最新修改时间 vs 二进制修改时间
  ...
  if [ -n "$src_mt" ] && [ "$src_mt" -gt "$bin_mt" ]; then
    need_build=1
    echo "[1/4] 检测到源码已更新, 重新构建联调版"
  else
    echo "[1/4] 二进制已是最新联调版, 跳过构建"
  fi
fi
