...
| **有效微步/转** | **6400** | 200 × 32 |
| 储药器（笔芯）内径 | 4.5 mm | 截面积 ≈ 15.904 mm² |
| 每转排开体积 | 7.952 µL | 面积 × 导程 |
| U-100 浓度 | 100 U / mL | 1U = 10µL |
| **微步/单位 (STEPS_PER_UNIT)** | **≈ 8048** | 6400 / 0.7952U |
| **每微步对应** | **≈ 0.000124 U (0.00124 µL)** | 远细于 0.05U 最小精度 |
| **0.05U 微步数** | **402 微步**（理论 402.4，取整） | 全系统最小剂量网格 |

> 结论：**打 0.05U = 402 微步**，实际剂量 0.04995U（误差 −0.10%，远低于医疗泵 ±15% 要求）。
> 机械分辨率 0.000124U 比 0.05U 精细约 400 倍，因此「控制精度」绰绰有余；真正决定
...
- 大剂量不再由 `motor_move_sync()` 一次跑完，而是 `motor_deliver_bolus()` 循环：
  - 每批 **`BOLUS_SEGMENT_UNITS` = 0.05U（402 微步）**；
