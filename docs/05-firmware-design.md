...
| **有效微步/转** | **6400** | 200 × 32 |
| 储药器（3ml 标准笔芯）内腔直径 | 9.65 mm | 截面积 ≈ 73.1 mm² |
| 每转排开体积 | 36.57 µL | 面积 × 导程 |
| U-100 浓度 | 100 U / mL | 1U = 10µL |
| **微步/单位 (STEPS_PER_UNIT)** | **≈ 1750** | 6400 / 3.657U |
| **每微步对应** | **≈ 0.000571 U (0.00571 µL)** | 远细于 0.05U 最小精度 |
| **0.05U 微步数** | **88 微步**（理论 87.5，取整 88） | 全系统最小剂量网格 |

> 结论：**打 0.05U = 88 微步**，实际剂量 0.05028U（误差 +0.56%，远低于医疗泵 ±15% 要求）。
> 机械分辨率 0.000571U 比 0.05U 精细约 88 倍，因此「控制精度」绰绰有余；真正决定
...
- 大剂量不再由 `motor_move_sync()` 一次跑完，而是 `motor_deliver_bolus()` 循环：
  - 每批 **`BOLUS_SEGMENT_UNITS` = 0.05U（88 微步）**；
