...
/* 字体 (UI 需要英文/数字, 启用 Montserrat 西文 + 默认)
 * 注意: 本模拟器用 LVGL 9.5.0 GitHub 源码的 CMake 构建, 其 lv_conf_internal.h
 * 以 CONFIG_LV_FONT_* / LV_FONT_* 内部宏为准 (非 Arduino 的 LV_USE_FONT_*),
 * 故此处直接用内部名 LV_FONT_MONTSERRAT_*。固件那份 lv_conf.h 仍用
 * LV_USE_FONT_*(Arduino 包期望), 二者构建系统不同, 不可混用。 */
